...
$query_lugares = "SELECT cantidad FROM cantidaddeparqueaderos";
$lugares = mysql_query($query_lugares, $prueba) or die(mysql_error());
$row_lugares = mysql_fetch_assoc($lugares);
$totalRows_lugares = mysql_num_rows($lugares);

mysql_select_db($database_prueba, $prueba);
$query_disponible = "SELECT * FROM disponibilidad WHERE disponibilidad.Disponibilidad='Verdadero'";
$disponible = mysql_query($query_disponible, $prueba) or die(mysql_error());
...
?>
        ...
        Nombre del empleado 
          ...
          <label for="select"></label>
          <select name="NombreEmpleado" id="select" class="form-control" required>
...
?>
    	...
    	Lugar asignado        
    	  ...
    	  <select name="Lugar" required class="form-control" >
			...
			for($i = 1; $i <= $limite;$i++)
			{
				$sql = "UPDATE disponibilidad SET Disponibilidad = 'Verdadero'";	
			}
do {  
?>
    	    <option value="<?php echo $row_disponible['Lugar']?>"><?php echo $row_disponible['Lugar']?></option>
    	    <?php
} while ($row_disponible = mysql_fetch_assoc($disponible));
  $rows = mysql_num_rows($disponible);
  if($rows > 0) {
      mysql_data_seek($disponible, 0);
	  $row_disponible = mysql_fetch_assoc($disponible);
  }
?>
          </select>
